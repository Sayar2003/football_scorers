...
export default function Fixtures() {
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '900px', margin: '0 auto' }}>
      <h1 style={{ fontSize: '24px', fontWeight: '700', color: dark.text, marginBottom: '1.5rem' }}>📅 Fixtures</h1>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.5rem' }}>
        <button onClick={() => changeDate(-1)} style={{
          ...
          backgroundColor: dark.card, color: dark.text, fontSize: '16px'
        }}>←</button>

        <div style={{ flex: 1, textAlign: 'center' }}>
          <div style={{ fontWeight: 'bold', fontSize: '18px', color: dark.text }}>
          ...
          <input
            ...
            style={{
              ...
              border: `1px solid ${dark.border}`, fontSize: '13px',
      ...
      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem', justifyContent: 'center' }}>
        {[-2, -1, 0, 1, 2].map(offset => {
          ...
          return (
            <button key={offset} onClick={() => setSelectedDate(dateStr)} style={{
              padding: '0.4rem 0.8rem', borderRadius: '8px', border: '1px solid',
              borderColor: selectedDate === dateStr ? dark.blue : dark.border,
              backgroundColor: selectedDate === dateStr ? dark.blue : dark.card,
              color: selectedDate === dateStr ? 'white' : dark.muted,
      ...
      {loading && <p style={{ textAlign: 'center', color: dark.muted }}>Loading fixtures...</p>}

      {!loading && matches.length === 0 && (
        <div style={{ textAlign: 'center', padding: '3rem', color: dark.muted }}>
          <p style={{ fontSize: '48px', marginBottom: '1rem' }}>😴</p>
          <p style={{ fontSize: '18px', color: dark.text }}>No matches on this date</p>
      ...
      {!loading && matches.map(match => (
        <div
          ...
          style={{
            ...
            justifyContent: 'space-between', padding: '0.9rem 1.5rem',
            marginBottom: '0.5rem', borderRadius: '10px',
            border: match.status === 'IN_PLAY' ? '1px solid #16a34a' : `1px solid ${dark.border}`,
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', width: '38%' }}>
            ...
            <span style={{ fontWeight: '500', fontSize: '14px', color: dark.text }}>
          ...
          <div style={{ textAlign: 'center', width: '24%' }}>
            {match.status === 'SCHEDULED' ? (
              <div>
                <div style={{ fontWeight: '600', fontSize: '15px', color: dark.text }}>{formatTime(match.utcDate)}</div>
                <div style={{ fontSize: '11px', color: dark.blue }}>Upcoming</div>
              </div>
            ) : (
              <div>
                <div style={{ fontSize: '20px', fontWeight: 'bold', color: dark.text }}>
                  {match.score.fullTime.home ?? '-'} : {match.score.fullTime.away ?? '-'}
                </div>
                <div style={{ fontSize: '12px', color: match.status === 'IN_PLAY' ? '#16a34a' : dark.muted }}>
